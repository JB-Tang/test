...
1. 设计模式：订阅与发布
    * 介绍
        * 定义了对象之间的一种一对多的依赖关系，当一个对象的状态发生变化时，所有依赖它的对象都可以得到通知
        * 与观察者模式概念相似，但是订阅-发布模式中，订阅者和发布者之间多了一层中间件：一个被抽象出来的信息调度中心
        * 特点：
        ...
        * 优缺点
            ...
            * 缺点：
                * 创建订阅者本身要消耗一定的时间和内存
                * 虽然可以弱化对象之间的联系，但多个发布者和订阅者嵌套在一起时，程序会变得难以跟踪维护
    * 代码示例
        * 代码
            ```javascript
            class Event{
                ...
                removeSub(name, userId) {
                    this.list[name] = this.list[name].filter(item => item.userId !== userId)
                }
            }
            ...
            */
            let event = new Event()
            // 订阅
            event.subscribe('人民日报', 'A', (content) => {
                console.log('A接收到人民日报推送的消息', content)
            })
            event.subscribe('湖北日报', 'B', (content) => {
                console.log('B接收到湖北日报推送的消息', content)
            })
            event.subscribe('人民日报', 'C', (content) => {
                console.log('C接收到人民日报推送的消息', content)
            })
            // 发布与取消订阅
            event.publish('人民日报', '最高礼遇！以国知名')     // A接收到人民日报推送的消息 最高礼遇！以国知名  C接收到人民日报推送的消息 最高礼遇！以国知名
            event.publish('湖北日报', '教育部明确规定')     // B接收到湖北日报推送的消息 教育部明确规定    
            event.removeSub('人民日报', 'A')
            event.publish('人民日报', '最新确诊消息')       // C接收到人民日报推送的消息 最新确诊消息
            ```
        * 解析
            * 上述代码中A,B,C相当于订阅者；人民日报，湖北日报相当于发布者
            * 每次订阅会将订阅者的信息存与调度中心（event实例的list）中，
            * 当发布信息时（如人民日报发送新闻），会从list中取出相关发布的信息数组（人民日报的订阅者数组）进行遍历执行回调函数。
2. Service Worker
    * 介绍
        * Service Worker是一个运行在后台的Worker线程，会长期运行，充当一个服务
        * 适合那些不需要网页或者用户互动的功能，常见的用途是拦截和处理网络请求
        * Service Worker可以修改用户的请求，或者直接向用户发出回应，不用联系服务器，这使得用户可以在离线情况下使用网络应用
        * Service Worker可以在本地缓存资源文件，直接从缓存加载文件，因此可以加快速度
        * 注意点
            * 为了节省内存，Service Worker在不使用的时候是休眠的，它不会保存数据，所以重新启动的时候，为了拿到数据，最好把数据放在IndexedDB里面。
            * Servive Worker是事件驱动的
    ...
    * 代码示例
        ...
        * 使用步骤
            ...
            * 与网页通信
                ...
                ```
    * 参考网址：https://www.bookstack.cn/read/webapi-tutorial/docs-service-worker.md
## 2022-10-31
1. 文件操作（File System Access API）
    * 介绍
        * 以File API为基础，提供文件读取功能，添加了新的接口，以支持修改文件以及使用目录
        * 目前该接口只在Chrome及chromium系的Edge浏览器上使用
        * 我们可以在页面文件上或者浏览器控制台上使用File System Access的相关方法
    * 具体使用
        * 打开文件
            * input方法
                * files返回的是一个File类型的对象数组
                * 代码示例
                    ```html
            ...
            * showOpenFilePicker方法
                * 介绍
                    * 该方法是异步的，调用后会弹出一个文件选择对话框，选择文件后会返回一个存有文件句柄的数组
                    * 获取文件句柄之后，需要调用getFile方法（异步），才可以获取到一个File（Blob）类型的对象
                    * getFile()获取的文件句柄可能会失效，删除或移动源文件会造成文件句柄对象失效，只能重新调用getFile()获取新的句柄。
                * 传参介绍
                    * multiple：布尔类型，默认值false,表示只能选择一个文件；true表示可以多选
                    * types：可选择的文件类型数组，支持以下两个参数
                        * description：表示文件或文件夹的描述，字符串类型，可选
                        * accept：接收的文件类型，对象，key为文件的MIME匹配，值是数组，表示支持的文件后缀
                * 代码示例
                    ```javascript
        ...
        * 新建文件并保存
            * 介绍
                * 新建文件，需要调用```showSaveFilePicker()```方法（异步），该方法会打开一个保存文件对话框
                * 可以通过传入参数来设置文件名称类型等，调用后返回新创建文件的句柄
                * 点击保存之后，新的文件就将被创建和保存，但是此时的文件是空的，需要接下来的写入操作
            * 代码示例
                ```javascript
        ...
        * 写入数据
            * 介绍
                * 要往文件里写入内容，需要调用文件句柄对象上的```createWritable()```来创建流
                * 由于从web写入文件到文件系统系统是一种不安全的行为，所以浏览器会询问我们是否授权
                * 注意
                    * 写入文件的过程是流操作，使用了```Stream```相关的API
                    * write()方法负责写入数据，可以是字符串，Blob对象，也可以是流
                    * 就像在node中使用管道一样，数据传输完成后，如果管道不会自动关闭，那么一定要手动关闭
            * 代码示例
                ```javascript
        ...
        * 使用默认文件名和默认目录
            * 介绍
                * 有时我们希望保存文件时有一个默认文件名或者能设置打开文件时所处的默认目录，这是我们就可以使用```suggestedName```、```startIn```这两个属性来实现
                * 常用的系统目录列表
                    * desktop：桌面
                    * documents：文档
                    * downloads：下载
                    ...
                    * videos：视频
                * 注意
                    * 除了常用的系统目录，我们也能传入自定义目录的句柄，自定义目录需要我们调用```showDirectoryPicker()```获得
            * 代码示例
                ```javascript
                async function addFile() {
                    const options = {
        ...
        * 打开目录并获取其内容
            * 介绍
                * 调用```showDirectoryPicker()```，我们可以选择要查看的目录，然后返回对应的目录句柄，使用它的```values()```方法我们就能查看目录中有哪些文件了
                * 注意
                    * 出于安全考虑，浏览器不允许我们访问一些敏感目录，例如包含系统文件的目录或者组策略不允许访问的目录
            * 代码示例
                ```javascript
        ...
        * 创建或访问目录中的文件与文件夹
            * 介绍
                * 在目录中，可以调用目录句柄的```getFileHandle()```和```getDirectoryHandle()```方法来访问文件和子目录
                * 同时可以传入一个```{create: true}```对象参数，表示当文件或目录不存在时创建它
            * 代码示例
                ```javascript
        ...
        * 删除文件或目录
            * 介绍
                * 当已经获取一个目录的句柄时，要想删除该目录或目录下的某个文件，可以调用```removeEntry()```方法
                * 默认条件下，只能删除空目录，否则会报错，但是可以传入一个对象```{recursive: true}```表示要递归删除目录下的所有子目录和文件
                * 如果删除不存在的目录或文件，返回的操作结果也是成功的
            * 代码示例
                ```javascript
                ...
                async function removeDir() {
                    let dirHandle = await window.showDirectoryPicker()
                    await dirHandle.removeEntry('oldDir', { recursive: true })
                }
                ```
        * 解析目录下文件的路径
            * 介绍
                * 通过目录句柄的```resolve()```方法，我们能获得一个表示文件所处路径的数组
                * 文件必须处于目录内，可以时目录的子目录
            * 代码示例
                ```javascript
                ...
                ```
    * 总结
        * 对比现有的File API, File System Access API确实更加强大，但是由于没有称为标准，未来甚至有可能被废弃，所以开发者需要谨慎使用。
        * 推荐库：```browser-fs-access```，提供了对File System Access API的封装，当浏览器支持时会优先使用，当浏览器不支持时会降级使用File API
    * 参考网址：https://juejin.cn/post/6991277058278047751#heading-2
## 2022-11-04
1. SPA应用与MPA应用
    * 单页应用（SPA）
        * 介绍
            * 第一次进入页面的时候会请求一个html文件，刷新清除一下
            * 切换到其他组件，路径发生变化，但是没有新的html文件请求，页面的内容也变化了
                * 原理：
                    * js会感知到url的变化，js动态的将当前页面的内容清除掉，然后将下一个页面的内容挂载到当前页面上
                    * 这个时候路由不是后端来做了，而是前端来做，判断页面到底是显示哪个组件，清除不需要的，显示需要的组件
            * 页面切换快的原因
                * 页面每次切换时，不需要做html文件的请求，这样就节约了很多http发送时延
        * 缺点
            * 首屏时间慢：首屏时需要请求一次html，同时还要发送一次js请求，两次请求回来了，首屏才会展示出来
            * SEO差：搜索引擎只认识html里的内容，不认识js的内容，而单页应用的内容都是靠js渲染生成出来的，搜索引擎不认识，就不会给一个好的排名
        * 注意
            * vue提供了一些其他技术来解决这些缺点，比如```服务器端渲染技术```
    * 多页应用（MPA）
        * 介绍
            * 每一次页面跳转时，后台服务器都会返回一个新的html文档，这种类型的网站就是多页网站
            * 由于访问页面的时候，服务器返回一个html，页面就会展示出来，这个过程只经历了一个HTTP请求
            * 搜索引擎在做网页排名时，要根据网页内容才能给网页权重，搜索引擎可以识别html,每个页面所有的内容都放在html中，所以这种多页应用，seo排名较好
        * 缺点
            * 切换慢：每次跳转都要发出一个http请求，如果网络比较慢，在页面中来回跳转时，就会发现明显的卡顿
    * SPA与MPA的区别
        |             | 多页面应用模式MPA | 单页面应用模式SPA |
        | ----------- | ---------------- | ---------------- |
        | 应用构成 | 由多个完成页面构成 | 一个外壳页面和多个页面片段构成 |
        | 跳转方式 | 页面之间的跳转是从一个页面跳转到另一个页面 | 页面之间的跳转是把一个页面片段删除或隐藏，加载另一个页面片段并显示出来，这是片段之间的模拟跳转，并没有开壳页面 |
        | 跳转后公共资源是否重新加载 | 是 | 否 |
        | URL模式 | ```http://xxx/page1.html```和```http://xxx/page2.html``` | ```http://xxx/shell.html#page1```和```http://xxx/shell.html#page2``` |
        | 用户体验 | 页面间切换加载慢，不流畅，用户体验差，特别是在移动设备上 | 页面片段之间切换快，用户体验好，包括在移动设备上 |
        | 能否实现转场动画 | 无法实现 | 容易实现（手机app动效） |
        | 页面之间传递数据 | 依赖URL、cookie或者localStorage，实现麻烦 | 因为在一个页面内，页面间传递数据很容易实现（比如vue中的vuex之类） |
        | 搜索引擎优化（SEO） | 可以直接做 | 需要单独方案做，有点麻烦 |
        | 特别适用的范围 | 需要对搜索引擎友好的网站 | 对体验要求高的应用，特别是移动应用 |
        | 开发难度 | 低一些，框架选择容易 | 高一些，需要专门的框架来降低这种模式的开发难度 |
    * 参考网址：https://juejin.cn/post/6844903929306693645
2. 前端路由实现原理
    * 介绍
        * 路由的工作：通过一定的机制，监听用户的行为动作，从而做出对应的变化
        * 通常在单页面（SPA）应用中，有两种方式实现路由
            * history模式和hash模式
    * history模式
        * 概念
            * history路由模式的实现，主要归功于HTML5中提供的history全局对象，主要依靠于```history.pushState()```和```history.replaceState()```
            * pushState与replaceState特点
                * 都会改变当前页面显示的URL，但都不会刷新页面
                * pushState是压入浏览器的会话历史栈中，会使history.length加1，而replaceState是替换当前的这条会话历史，因此不会增加history.length
            * 事件处理程序```popstate```
                * history.pushState和history.replaceState方法不会触发popstate事件
                * 浏览器的某些行为：go,back,forward会导致popstate
                * popstate事件对象中的state属性，可以理解成pushState和replaceState方法时的第一个参数
        * 实现方法
            * 方法一
                * 针对popstate事件只有在go,back,forward情况下才会触发，我们可以考虑额在pushState或replaceState时多pushState一份，之后再history.go(-1)
                * 代码实现
                    ```javascript
                    window.addEventListener('popstate', e => {
                        console.log('监听到url变化', e)
                    })
                    // pushState
                    ...
                    function replacePage(page) {
                        history.replaceState({pageUrl: page}, null, page)
                        history.pushState({pageUrl: page}, null, page)
                        history.go(-1)
                    }
                    ```
            * 方法二
                * 对于popstate事件无法监听pushState与replaceState的情况，我们可以考虑对pushState与replaceState事件进行重写，添加事件监听
                * 代码实现
                    ```javascript
                    ...
                    window.addEventListener('pushState', e => {
                        console.log('触发的pushState事件：', e)
                    })
                    window.addEventListener('replaceState', e => {
                        console.log('触发的replaceState事件：', e)
                    })
                    ```
        * 重点 
            * hash模式不需要后端配合，但是history模式下，如果跳转路由后刷新就会得到404的错误，所以此时需要后台配置当页面404时的默认地址（通常用nginx实现）
    * hash模式
        * 概念
            * 在一串url中，#后面的，就是一个URL中关于hash的组成部分
            * 在浏览器中已经暴露给我们一个现成的方法```hashchange```，通过这个事件，我们就可以监听hash的变化，在回调函数中展示和隐藏不同的UI功能
        * 总结
            * hash模式所有工作都是在前端完成的，不需要后端服务的配合
            * hash模式的实现方式就是通过监听URL中hash部分的变化，从而做出对应的渲染逻辑
            * hash模式下，URL中会带有#，看起来不太美观
    * 总结
        * 一般路由的实现主要有history和hash两种方式
        * hash的实现全部在前端，不需要后端服务器配合，兼容性好，主要通过监听```hashchange```事件，处理前端业务逻辑
        * history的实现，需要服务器做一下简单的配置，通过监听```pushState```和```replaceState```事件，处理前端业务逻辑
    * 参考网址：
        * https://juejin.cn/post/6844903586023866381
        * https://juejin.cn/post/7127143415879303204
## 2022-11-08
1. 派发自定义事件dispatchEvent
    * 介绍
        * 标准浏览器中使用```dispatchEvent```派发自定义事件
        * 一般自定义事件的流程： 创建 -> 初始化 -> 派发
    * 自定义事件流程
        * 创建：```document.createEvent()```
            * createEvent(eventType)方法返回新创建的Event对象
            * eventType共有5中类型
                * Events：表示全部的事件
        ...
        * 初始化：
            * 在createEvent后必须初始化，根据不同的eventType初始化方法也不同
            ...
            * 传参介绍
                * 'type'：可以是固定的名称，例如：'click', 'blur'，也可以是自定义的名称，例如：'myEvent'
                * bubbles：是否可以冒泡，boolean类型
                * cancelable：是否阻止事件的默认操作，boolean类型
        * 派发：```element.dispatchEvent()```
            * dispatchEvent(eventObject)就是派发自定义事件了
            * eventObject：表示事件对象
            * 注意：在IE上请使用fireEvent方法
        * 代码示例
            ```html
            <div id="div"></div>
            <script>
            ...
            ```
    * 注意点
        * 可以直接使用Event或CustomEvent去定义自定义事件，从而取代创建和初始化流程
        * 代码示例
            ```html
            <div id="div"></div>
            <script>
    ...
    * 参考网址
        ...
        * http://www.javashuo.com/article/p-yvamqpgu-ga.html
2. IndexedDB
    * 介绍
        * IndexedDB是一个运行在浏览器上的非关系型数据库，理论上来讲，IndexedDB是没有存储上限的，一般来说不会小于250M(相比较于webStorage的4M)。
        * IndexedDB不仅可以存储字符串，还可以存储二进制数据
    * 特点
        * 键值对存储：
            * IndexedDB内部采用对象仓库存放数据，所有类型的数据都可以直接存入，包括javascript对象
            * 对象仓库中，数据以“键值对”的形式保存，每一个数据记录都有对应的主键，主键是独一无二的，不能有重复，否则会抛出一个错误
        * 异步操作
            * IndexedDB操作时不会锁死浏览器，用户依然可以进行其他操作，这与LocalStorage形成对比，后者的操作是同步的。
            * 异步设计是为了防止大量数据的读写，拖延网页的表现
        * 支持事务
            * IndexedDB支持事务，这意味着一系列的操作步骤中，只要有一步失败，整个事务就都取消，数据库回滚到事务发生之前的状态，不存在只改写一部分数据的情况
        * 同源限制
            * IndexedDB受到同源限制，每一个数据库对应创建它的域名。网页只能访问自身域名下的数据库，而不能访问跨域的数据库
        * 支持二进制存储
            * IndexedDB不仅可以存储字符串，还可以存储二进制数据（ArrayBuffer对象和Blob对象）
        * 存储空间大
            * IndexedDB存储空间比LocalStorage大得多，一般来讲不少于250M，甚至没有上限
            * 存储在电脑上得位置```C:\Users\当前的登录用户\AppData\Local\Google\Chrome\User Data\Default\IndexedDB```
    * 相关概念
        * 数据库：IDBDatabase对象
            * 数据库是一系列相关数据得容器
            * 每个域名（协议+域名+端口）都可以新建任意多个数据库
            * IndexedDB有版本得概念
                * 同一个时刻，只能有一个版本得数据库存在
                * 如果要修改数据库接口（新增或删除表、索引或者主键），只能通过升级数据库版本完成
        * 对象仓库：IDBObjectStore对象
            * 每个数据库包含若干个对象仓库，类似于关系型数据库中的表格
        * 索引：IDBIndex对象
            * 为了加速数据的检索，可以在对象仓库里面，为不同的属性建立索引
        * 事务：IDBTransaction对象
            * 数据记录的读写和删改，都要通过事务完成。事务对象提供error,abort和complete三个事件监听操作结果
        * 操作请求：IDBRequest对象
            * 数据库的操作都是通过这个对象完成的
        * 指针：IDBCursor对象
            * 用来遍历数据仓库或索引的记录
        * 主键集合：IDBKeyRange对象
            * 代表数据仓库里面的一组主键。根据这组主键，可以获取数据仓库或索引里面的一组记录
    * 操作实例
        * 打开数据库
            ```javascript
            ...
            * 注意点
                ...
                * 第一次新建库是也会触发数据库升级事件，因为数据仓库从无到有算是升级了一次
        * 新建数据库
            * 新建数据库与打开数据库是同一操作
        * 新建对象仓库(表)
            ```javascript
            ...
            * 注意点
                * 新建之前最好判断一下表存不存在
                * 主键（key）是默认建立索引的属性，主键可以使用当前层，也可以指定对象下一层对象的属性
                * 如果数据记录中没有合适作为主键的属性，那么可以让IndexedDB自动生成主键
        * 新建索引
            * 新建索引可以使用```IDBObject.createIndex```，对应有三个参数分别为索引名称，索引所在的属性，配置对象（说明该属性是否包含重复的值）
            ```javascript
            // 接上面数据库升级事件
            request.onupgradeneeded = function (res) {
                db = res.target.result
        ...
        * 新增数据
            * 新增数据指的是向对象仓库写入数据记录，这需要通过事务完成
            ...
            ```
            * 注意点
                * 新建事务时需要指定表格名称和操作模式（只读或读写）
                * 写入操作是一个异步操作，通过监听连接对象的success事件和error事件，了解是否写入成功
        * 读取数据
            * 读取数据也是通过事务完成，使用```objectStore.get()```方法读取数据，参数是主键的值
            ```javascript
            function readData() {
        ...
        * 更新数据
            ...
            * 注：put不仅可以修改，也可以新增
        * 删除数据
            * 删除数据使用```IDBObjectStore.delete()```方法
            ```javascript
            function removeDate() {
            ...
            ```
        * 使用索引
            * 索引的意义在于可以搜索任意字段，如果不建立索引，那么默认只能搜索主键
            * 上述我们在建对象仓库时创建了一个name字段的索引，使用方法如下
            ```javascript
        ...
        * 遍历数据
            * 遍历表格的所有记录，要使用指针对象IDBCursor
            ...
            ```
    * 参考网址
        * https://juejin.cn/post/7025911892056997924
        * https://www.bookstack.cn/read/javascript-tutorial/docs-bom-indexeddb.md
## 2022-11-14
1. vue-项目中public文件夹与assets文件夹的区别
    * public文件夹下面的静态资源会原封不动的放到编译后的dist文件夹下
    * assets文件夹下的静态资源会被编译之后放到dist文件夹下
    * 注意点
        * 使用public文件夹中的文件要使用绝对路径，使用assets中的静态资源要参照相关的打包工具使用具体的引入方式
        * assets文件夹中适合放经常变动的资源，public文件夹适合放不会变动的资源（因为浏览器的缓存机制。同名文件不会被二次请求，这会导致public中的文件更新时浏览器不会更新，至于assets文件夹中的文件由于编译时加入了hash值，所以文件名改变了，浏览器会重新下载）
        * 引用assets中不存在的文件在编译时就会报错，引用public中不存在的文件只有在客户端使用时才会报404错误
2. css预处理器
    * 传统css弊端
        * 没有变量，不支持嵌套，编程能力较弱，代码复用性差，对于新的属性需要考虑兼容性
    * 常见功能：变量、嵌套、函数、混入（mixin）
    * 常见的css预编译器：sass/scss、less、stylus
## 2022-12-06
1. base64
    * 介绍
        * base64构成：```A-Z```、```a-z```、```0-9```、```+```、```/```，注意，Safe base64中为了避免字符与路劲符号产生冲突，将 ```+``` 替换成 ```_``` , ```/``` 替换成 ```-```
        * base64是一种基于64个可打印字符来标识二进制数据的表示方法，由于6个比特可以代表64个字符，所以每6个比特为一个单元，对应某个可打印字符。3个字节相当于24个比特，对应4个base64单元，即3个字节可以由4个可打印字符表示
        * base64常用于在处理文本数据的场合，表示、传输、存储一些二进制数据，包括MIME的电子邮件及XML的一些复杂数据
    * 参考网址
        * https://juejin.cn/post/7082787017330950174#heading-11
2. 内存泄漏
    * 介绍
        * 内存泄漏：简单来讲，就是某块内存虽然不再使用，但是无法释放
        * 一些“无用”的内存得不到应有的释放，其占据的内存空间不断加大，最终导致可用内存空间不足，造成了程序崩溃
    * 产生原因
        * 由内存管理中的垃圾回收机制引起的
        * js通过自动内存管理实现内存分配和闲置资源回收，基本思路：确定哪个变量不会再使用,然后释放它占有的内存，这个过程是周期性的
        * 内存泄漏的罪魁祸首，就是错误的引用，开发者将无用的变量，添加为生存中的变量的引用，导致这个“无用的变量”无法被标记回收，对应的内存空间也无法释放
    * 危害
        * 导致可用内存空间不足
        * 导致与之关联的内存无法释放
    * 常见内存泄漏问题以及解决方法
        |  类型  |  说明  |  解决方法  |
        | ----- | ------ | --------- |
        | addEventListener | 最常见的一种。添加再window、body等全局对象上的事件监听器，销毁组件时如果忘记移除，就会导致内存泄漏。而添加在组件 DOM 上的事件监听器则不需要手动销毁，因为 DOM 在销毁的时候，事件监听器也会自行移除。 | 由于事件监听器移除时，要求 type、listener，useCapture 三个参数都是一致的，也就是事件类型、监听器函数、是否为捕获监听器都需要保持一致。否则移除将会失败，同样无法解决内存泄漏问题，所以使用调用addEventListener时配置的参数去调用removeEventListener |
        | setTimeout / setInterval |  | 调用clearTimeout/clearInterval清除 |
        | InterseectionObserver / ResizeObserver / MutationObserver | 在组件内部创建了一个观察器，并绑定到一个全局变量（body, document, footer, header）上 | 在组件销毁时，你需要调用disconnect()来清除 |
        | Promise / Observables / EventEmitters | 如果忘记停止监听，任何用于设置监听器的编程模型都有可能造成内存泄漏 |  |
        | 全局对象存储，如Redux,vuex |  | 为它追加内存，永远不会被清理 |
        | 无限增长的DOM | 在没有虚拟化的情况下，实现无限滚动列表，那么DOM节点的数量将无限制地增长 |  |
        | console.log | 被console.log打印的对象，在没有清空控制台的情况下，是不会进行垃圾回收的 | 减少生产环境的console.log，比如可以在webpack中配置自动移除console.log语句的选项 |
        | URL.createObjectURL | 当每次调用createObjectURL()方法时，都会创建一个新的URL对象，即使你已经用相同的对象作为参数创建过 | 当不再需要这些URL对象时，每个对象必须通过调用```URL.revokeObjectURL()```方法来释放
    * 参考网址
        * https://juejin.cn/post/7163152277056782350
# 2023-02-01
1. postMessage
    * 定义
        * postMessage是html5引入的API,postMessage()方法允许来自不同源的脚本采用异步方法进行有效的通信，可以实现跨文本文档，多窗口，跨域消息传递，多用于窗口间数据通信，这也使它成为跨域通信的一种有效的解决方案
        * postMessage类似于Ajax，但是它不受同源策略的限制，并且通信双方都是客户端
    * API介绍
        * 发送数据
            * 发送数据格式
                ```quoteWindow.postMessage(message, targetOrigin, [transfer])```
            * 参数介绍
                * quoteWindow: 表示其他窗口的一个引用，是被发送数据的一方（接收数据），可以是以下类型：
                    * iframe的contentWindow
                        ```html
                        <body>
                            <iframe src="http://xxx:8080" class="childIframe"></iframe>
                            <script>
                                let quoteWindow = document.querySelector('.childIframe').contentWindow
                            </script>
                        </body>
                        ```
                    * 执行window.open()方法返回的对象
                        ```html
                        <script>
                            let quoteWindow = window.open('http://xxx:8080')
                        </script>
                        ```
                    * 命名过或数值索引的window.frames
                        ...
                        <script>
                            ...
                            let quoteWindow = frames[0] 
                        </script>
                        ```
                * message: 要发送到其他窗口的数据，它将会被【结构克隆算法】序列化，这意味着你可以不受什么限制的将数据对象安全的传送给目标窗口而无需自己序列化
                * targetOrigin: 指定哪些窗口可以接收到该消息事件，可以是以下值：
                    * '*'：任何窗口
                    * '/': 当前窗口的同源窗口
                    * URL：表示只有该URL地址的窗口才能接收到消息，该设计可以保护敏感信息，防止恶意攻击
                * transfer: 可选属性，一串和message同时传递的Transferable对象，这些对象的所有权将被转移给消息的接收放，而发送的一方将不再保留所有权
        * 接收数据
            * 监听message事件的发生
            * 代码示例
                ```javascript
                ...
                * event对象参数解析
                    * data: 从其他窗口发送过来的消息对象
                    * type: 发送消息的类型
                    * source: 发送消息的窗口对象，可以用来向发送消息的窗口返回数据
                    * origin: 发送消息窗口的源，可以通过此属性判断源是否安全
    * 代码示例
        * 父页面
            ```html
            <!DOCTYPE html>
            ...
            <head>
                ...
                <meta http-equiv="X-UA-Compatible" content="IE=edge">
                <meta name="viewport" content="width=device-width, initial-scale=1.0">
                <title>postMessage验证父页面</title>
            </head>
            <body>
                <textArea id="parentTextArea"></textArea>
                <button onclick="sendMessageToChild()">发送</button>
                <div id="childText"></div>
                <iframe id="childIframe" src="http://127.0.0.1:5500/Test-Study/study-code/simple-code/testPostMessageChild.html" frameborder="1"></iframe>
            </body>
            <script>
            ...
            function sendMessageToChild() {
                let text = document.querySelector('#parentTextArea').value
                if (text) {
                    document.querySelector('#childIframe').contentWindow.postMessage(text, '*')
                } else {
                    alert('请先在父页面输入框中输入内容')
                }
            }
            </script>
            </html>
        ...
        * 子页面
            ...
            <head>
                ...
                <meta http-equiv="X-UA-Compatible" content="IE=edge">
                <meta name="viewport" content="width=device-width, initial-scale=1.0">
                <title>postMessage验证子页面</title>
            </head>
            <body>
                <div>这是嵌套的子页面</div>
                <button onclick="returnChildGet()">发送返回消息</button>
                <div id="childOutText"></div>
            </body>
            <script>
            window.addEventListener('message', e => {
                console.log('获取到的其他页面发送的消息', e)
                document.querySelector('#childOutText').innerHTML += '<br>' + e.data
            })
            function returnChildGet() {
                window.parent.postMessage('子页面确认接收到数据', '*')
            }
            </script>
            </html>
            ```
    * 使用场景
        * 跨域通信
            * 使用JSONP可以实现解决GET请求跨域的问题，但是不能解决POST请求跨域的问题，而使用postMessage就可以实现GET与POST请求跨域问题
        * WebWorker
        * Service Worker
    * 参考网址
        * https://juejin.cn/post/6844903665694687240
        * https://juejin.cn/post/6844904040216674311
# 2023-02-02
1. 
...
* websocket
* 微前端：
    https://juejin.cn/post/7155266023568965640
    https://juejin.cn/post/7110803251183747109
* 微模块
    https://juejin.cn/post/7134889749738684447
* 设计模式
...
## 学习路线
1. [前端八股文](https://juejin.cn/post/7016593221815910408)
2. [8年前端开发的知识点沉淀(不知道会多少字，一直写下去吧...)](https://juejin.cn/post/6844903870276042759)
3. [前端面试题整理—JavaScript篇](https://www.cnblogs.com/wjian0916/articles/15109038.html)
4. [前端学习路线](https://objtube.gitee.io/front-end-roadmap/#/)
5. [30 道 Vue 面试题，内含详细讲解（涵盖入门到精通，自测 Vue 掌握程度）](https://juejin.cn/post/6844903918753808398)
